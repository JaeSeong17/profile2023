    ...
    reviews: [
      {
        ...
        독창적인 의도에 맞게 구현해내고 과정에서 마주하는 에러를 해결해내는 힘은 원리를 이해하는 것에서 나온다고 느낍니다. \
        예제를 통해 라이브러리의 사용법을 빠르게 익히고 나면 어떤 방식으로 동작하는지 살',
      },
      {
        title: '과한 애니메이션과 화려한 UI는 사용성을 낮춘다?',
        content:
          '부드럽고 연결성 있는 애니메이션과 다양한 사용자 상호작용은 매력적이지만, \
          직접 사용하는 입장에서는 단순하고 직관적인 UI를 원할 수 있습니다. \
          Threejs, gsap으로 독특하게 화면을 구성했지만 실제로 사용해보면 \
          글 조회 목적인 게시판에는 적합하지 않다는 느낌을 받을 수 있습니다. \
          빠른 데이터 제공, 응답과 처리 속도, 화려한 디자인과 인터렉티브한 UI를 각 목적에 맞게 적절히 사용하는 것이 중요합니다.',
      },
    ],
    issues: [
      {
        ...
        content:
          'React-three-fiber는 Threejs를 React에서 선언형으로 사용하기 쉽게 만든 라이브러리입니다. \
            React의 컴포넌트가 DOM트리로 연결되듯 Threejs의 구성요소도 DOM트리에 연결 된다고 판단했습니다. 하지만 3D\
            화면상의 Mesh들의 상태를 루트 노드에 연결된 Redux 스토어에서 전역으로 관리하려 하다보니 Threejs의 렌더링 화면 \
            내에선 전역상태에 접근하지 못하는 문제가 발생했습니다. React-three-fiber는 형태만 react와 유사할 뿐 \
            Threejs를 렌더링하는 화면 내 구성요소는 화면 외부 DOM 트리와는 별도로 관리됩니다. \
            Threejs Scene 내부의 HTML 요소도 마찬가지로 외부 DOM트리와는 분리되어 있기 때문에 \
    ...
    reviews: [
      {
        ...
        content:
          ...
          프로젝트에서 관리할 수 있게 되면서 프론트와 백으로 나누어 생각하던 관점과 충돌이 있었습니다. 특히 서버 컴포넌트,\
           ...
           했습니다. 정리된 글을 보며 빠르게 지식을 습득하려던 습관을 멈추고 공식문서의 Data Fetching과 Component Rendering \
           가이드를 읽으니 Nextjs 동작 방식에 대한 이해를 높일 수 있었습니다..',
      },
      {
        title: '모든 방면에서 우수한 하나의 툴은 없다',
        content:
          '라이브러리나 툴을 사용할 때 목적을 생각하고 장단점을 살펴보는 안목을 기르게 되었습니다. 이전 프로젝트에서 사용한 \
          styled-component는 별도의 CSS 파일을 생성하지 않고 컴포넌트 단위로 스타일 관리에 용이해 선언형 프로그래밍 React와 \
          어울린다고 판단했습니다. 하지만 JS번들에 포함되는 점에 의해 SSR에 적합하지 않았고 다른 CSS 라이브러리를 찾아보게 되는 \
          계기가 되었습니다. Tailwind의 경우 빌드타임에 스타일을 적용하는 방식으로 SSR과 잘 어울리지만 CSS-In-JS 처럼 동적 \
          변수 사용에 제한이 있다는 점을 보면서 특정 라이브러리에 한정하지 않고 상황에 맞는 적절한 선택의 필요성을 느꼈습니다.',
      },
    ],
    issues: [
      {
        ...
        content:
          ...
          동적으로 반응하는 부분에서는 클라이언트 컴포넌트를 적절히 사용했습니다.',
      },
      {
        ...
        content:
          ...
          react-query의 훅으로 간편하게 제어할 수 있었습니다. ',
      },
      {
        title: '반응형 웹',
        content:
